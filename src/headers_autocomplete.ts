...
class IncludeCompletionProvider implements vscode.CompletionItemProvider, vscode.Disposable {
  ...
  public provideCompletionItems(document: vscode.TextDocument, position: vscode.Position,
    ...
    let exts = [
		...
		".hpp",
		".interface"
	];//vscode.workspace.getConfiguration("include-autocomplete").get("extensions", []);
    ...
    if (separator !== -1) {
      dirs = dirs.map(dir => join(dir, contents.substr(0, separator)));
		...
		const targetFileName = "engine.hpp";
		let item = new vscode.CompletionItem("engine.hpp",vscode.CompletionItemKind.Module);
		let curfile = document.uri.fsPath;
		item.insertText = "relativessss";
		item.detail = "here placed project relative path"
		...
		if (workspaceFolder)
		{
			...
			let currentDir = dirname(curfile);
			const files = fs.readdirSync(currentDir).filter(file => file === targetFileName);
			if (files.length > 0) {
				const relativePath = relative(workspacePath, join(currentDir, files[0]));
				item.insertText = relativePath;
			} else {
				item.insertText = "fucked";
			}
    ...
    const promises = dirs.map(async dir => {
      ...
      return unseen.reduce((items: vscode.CompletionItem[], entry: string) => {
        ...
        } else if (exts.indexOf(extname(entry).toLocaleLowerCase()) !== -1) {
  ...
  private async updateDirs() {
	...
	const filename = join(workspace, ".vscode/c_cpp_properties.json");
    ...
    if (properties) {
      const config = properties.configurations.find((c: { name: string; }) => c.name === platform);
    ...
    dirs = dirs.map(dir => {
      return dir.replace("${workspaceRoot}", workspace)
...
function readFile(filename: string, encoding: string): Promise<string> {
  return new Promise((c, e) => fs.readFile(filename, "utf-8", (err, data) => err ? e(err) : c(data)));
}
